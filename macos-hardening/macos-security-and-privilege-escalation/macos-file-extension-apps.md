# macOS Dosya Uzantısı & URL şeması uygulama yöneticileri
...
**`/usr/libexec/lsd`** veritabanının beyinidir. **Birçok XPC hizmeti** sağlar, örneğin `.lsd.installation`, `.lsd.open`, `.lsd.openurl` ve daha fazlası. Ancak, aynı zamanda **açık XPC işlevselliklerini kullanabilmek için uygulamalara bazı yetkilendirmeler** gerektirir, örneğin mime türleri veya url şemaları için varsayılan uygulamaları değiştirmek üzere `.launchservices.changedefaulthandler` veya `.launchservices.changeurlschemehandler` gibi.
