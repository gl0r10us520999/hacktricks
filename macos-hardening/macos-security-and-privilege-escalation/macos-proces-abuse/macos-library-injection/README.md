...
이 기술은 모든 설치된 애플리케이션에 "Info.plist"라는 plist가 있어 **환경 변수를 할당**할 수 있는 `LSEnvironmental`이라는 키를 사용하여 **ASEP 기술로도 사용될 수 있습니다**.

{% hint style="info" %}
2012년 이후로 **Apple은 `DYLD_INSERT_LIBRARIES`의 기능을 크게 제한했습니다**.
...
* 소프트웨어에 [`com.apple.security.cs.allow-dyld-environment-variables`](https://developer.apple.com/documentation/bundleresources/entitlements/com\_apple\_security\_cs\_allow-dyld-environment-variables) 권한이 없는 권한이 있습니다.
...
또는 바이너리가 **하드닝된 런타임 플래그** 또는 **라이브러리 검증 플래그**를 **가지지 않아야** 합니다.

바이너리가 **하드닝된 런타임**을 가지고 있는지 확인하려면 `codesign --display --verbose <bin>`을 사용하여 **`CodeDirectory`**에서 런타임 플래그를 확인하세요: **`CodeDirectory v=20500 size=767 flags=0x10000(runtime) hashes=13+7 location=embedded`**

바이너리가 **같은 인증서로 서명된 경우** 라이브러리를 로드할 수도 있습니다.
...
{% hint style="danger" %}
이전 라이브러리 검증 제한이 Dylib 하이재킹 공격을 수행하는 데에도 적용된다는 점을 기억하세요.
{% endhint %}

Windows와 마찬가지로 MacOS에서도 **dylibs를 하이재킹**하여 **애플리케이션**이 **임의의** **코드를 실행**하도록 만들 수 있습니다(사실 일반 사용자에게는 TCC 권한이 필요할 수 있으므로 `.app` 번들 내에서 쓰기 위해 라이브러리를 하이재킹하는 것은 불가능할 수 있습니다).\
...
이 기능과 관련된 **주요** 코드는 **`ImageLoader::recursiveLoadLibraries`**에 있습니다.
...
{% hint style="danger" %}
이전 라이브러리 검증 제한이 Dlopen 하이재킹 공격을 수행하는 데에도 적용된다는 점을 기억하세요.
{% endhint %}

**`man dlopen`**에서:

* 경로에 **슬래시 문자가 포함되지 않으면**(즉, 단순한 리프 이름인 경우) **dlopen()은 검색을 수행합니다**. **`$DYLD_LIBRARY_PATH`**가 시작 시 설정된 경우, dyld는 먼저 **해당 디렉토리**를 **검색합니다**. 다음으로, 호출된 macho 파일이나 주 실행 파일이 **`LC_RPATH`**를 지정하면 dyld는 **해당 디렉토리**를 **검색합니다**. 다음으로, 프로세스가 **제한되지 않은 경우**, dyld는 **현재 작업 디렉토리**를 검색합니다. 마지막으로, 오래된 바이너리의 경우, dyld는 몇 가지 대체 경로를 시도합니다. **`$DYLD_FALLBACK_LIBRARY_PATH`**가 시작 시 설정된 경우, dyld는 **해당 디렉토리**를 검색하고, 그렇지 않으면 dyld는 **`/usr/local/lib/`**(프로세스가 제한되지 않은 경우)에서 검색한 다음 **`/usr/lib/`**에서 검색합니다(이 정보는 **`man dlopen`**에서 가져온 것입니다).
1. `$DYLD_LIBRARY_PATH`
2. `LC_RPATH`
3. `CWD`(제한되지 않은 경우)
...
* 프로세스가 **제한되지 않은 경우**, CWD의 상대 경로를 악용하여 언급된 환경 변수를 사용하는 것입니다(문서에 명시되어 있지 않더라도 프로세스가 제한된 경우 DYLD\_\* 환경 변수는 제거됩니다).
{% endhint %}

* 경로에 **슬래시가 포함되어 있지만 프레임워크 경로가 아닌 경우**(즉, dylib에 대한 전체 경로 또는 부분 경로), dlopen()은 먼저 (설정된 경우) **`$DYLD_LIBRARY_PATH`**에서 검색합니다(경로의 리프 부분). 다음으로, dyld는 **제공된 경로를 시도합니다**(제한되지 않은 프로세스의 경우 상대 경로에 대해 현재 작업 디렉토리를 사용). 마지막으로, 오래된 바이너리의 경우, dyld는 대체 경로를 시도합니다. **`$DYLD_FALLBACK_LIBRARY_PATH`**가 시작 시 설정된 경우, dyld는 해당 디렉토리에서 검색하고, 그렇지 않으면 dyld는 **`/usr/local/lib/`**(프로세스가 제한되지 않은 경우)에서 검색한 다음 **`/usr/lib/`**에서 검색합니다.
1. `$DYLD_LIBRARY_PATH`
2. 제공된 경로(제한되지 않은 경우 상대 경로에 대해 현재 작업 디렉토리 사용)
...
```
만약 당신이 그것을 컴파일하고 실행하면 **각 라이브러리가 어디에서 실패했는지** 볼 수 있습니다. 또한, **FS 로그를 필터링할 수 있습니다**:
```bash
...
파일 `dyld-dyld-832.7.1/src/dyld2.cpp`에서 **`pruneEnvironmentVariables`** 함수가 있어, **`DYLD_`**로 시작하는 모든 환경 변수와 **`LD_LIBRARY_PATH=`**를 제거할 수 있습니다.
...
```
어떤 의미에서 이진 파일이 **suid** 또는 **sgid**이거나 헤더에 **RESTRICT** 세그먼트가 있거나 **CS\_RESTRICT** 플래그로 서명된 경우, **`!gLinkContext.allowEnvVarsPrint && !gLinkContext.allowEnvVarsPath && !gLinkContext.allowEnvVarsSharedCache`**가 true가 되고 환경 변수는 제거됩니다.
...
{% hint style="danger" %}
**`0x0(none)`** 플래그로 서명된 바이너리가 있더라도, 실행될 때 동적으로 **`CS_RESTRICT`** 플래그를 가질 수 있으므로 이 기술은 그들에 대해 작동하지 않습니다.
