...
```
然而，这将在执行的应用程序上触发错误，另一种更隐蔽的方法是创建一个 Java 代理并使用：
```bash
...
```
注意，在这个例子中，Android Studio 正在尝试加载文件 **`/Applications/Android Studio.app.vmoptions`**，这是任何来自 **`admin` 组的用户都有写入权限的地方。**
