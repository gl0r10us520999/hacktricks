...
Дивіться, як **отримати цю інформацію легко за допомогою `lldb` в ARM64** на цій сторінці:
...
[**Dynadump**](https://github.com/DerekSelander/dynadump) - це інструмент для класового дампу бінарних файлів Objective-C. GitHub вказує на dylibs, але це також працює з виконуваними файлами.
```bash
...
* Отримати власного клієнта з трасування з KERN\_KDPINDEX
...
{% hint style="info" %}
При виклику функції **`objc_sendMsg`** регістр **rsi** містить **назву методу** як рядок з нульовим закінченням (“C”). Щоб надрукувати назву через lldb, виконайте:
...
* Ви можете перевірити, чи функція **`sysctl`** або **`ptrace`** імпортується (але шкідлива програма може імпортувати її динамічно)
...
* Обмеження `AS_CORE` дозволяє цю операцію. Можна подавити створення дампів, викликавши `ulimit -c 0` і знову увімкнути їх за допомогою `ulimit -c unlimited`.
...
Якщо ви фуззите через SSH-з'єднання, важливо переконатися, що сесія не закінчиться. Тому змініть файл sshd\_config на:
