...
Зверніть увагу, що якщо пакет .app вже був авторизований для запуску (він має атрибут карантину з прапором авторизації на запуск), ви також можете зловживати цим... за винятком того, що тепер ви не можете записувати всередині пакетів **`.app`**, якщо у вас немає деяких привілейованих прав TCC (яких у вас не буде всередині пісочниці).
{% endhint %}
...
Як пояснюється в [**цьому пості**](https://www.vicarius.io/vsociety/posts/cve-2023-26818-sandbox-macos-tcc-bypass-w-telegram-using-dylib-injection-part-2-3?q=CVE-2023-26818), якщо ви хочете отримати стійкість з програмою, яка є пісочницею, ви можете зробити так, щоб вона автоматично виконувалася як LaunchAgent і, можливо, впровадити шкідливий код через змінні середовища DyLib.
...
Sandbox Bypassed!
```
#### Інтерпозувати `__mac_syscall`, щоб запобігти пісочниці
...
```
{% hint style="warning" %}
**Навіть з обхідним шляхом Sandbox TCC** запитає користувача, чи хоче він дозволити процесу читати файли з робочого столу
{% endhint %}
