...
```
Concernant les **extensions du noyau**, le dossier `/var/db/SystemPolicyConfiguration` contient des fichiers avec des listes de kexts autorisés à être chargés. De plus, `spctl` a le droit `com.apple.private.iokit.nvram-csr` car il est capable d'ajouter de nouvelles extensions du noyau pré-approuvées qui doivent également être enregistrées dans la NVRAM dans une clé `kext-allowed-teams`.
...
Ce Kext va intercepter via MACF plusieurs appels afin de capturer tous les événements du cycle de vie des fichiers : Création, ouverture, renommage, liaison dure... même `setxattr` pour l'empêcher de définir l'attribut étendu `com.apple.quarantine`.
...
Cependant, cela n'est plus possible car macOS **empêche la modification des fichiers** à l'intérieur des bundles d'applications. Donc, si vous essayez l'attaque [Dirty NIB](../macos-proces-abuse/macos-dirty-nib.md), vous constaterez qu'il n'est plus possible d'en abuser car après avoir exécuté l'application pour la mettre en cache avec Gatekeeper, vous ne pourrez pas modifier le bundle. Et si vous changez par exemple le nom du répertoire Contents en NotCon (comme indiqué dans l'exploit), puis exécutez le binaire principal de l'application pour le mettre en cache avec Gatekeeper, cela déclenchera une erreur et ne s'exécutera pas.
...
Dans ce contournement, un fichier zip a été créé avec une application commençant à se compresser à partir de `application.app/Contents` au lieu de `application.app`. Par conséquent, l'**attribut de quarantaine** a été appliqué à tous les **fichiers de `application.app/Contents`** mais **pas à `application.app`**, que Gatekeeper vérifiait, donc Gatekeeper a été contourné car lorsque `application.app` a été déclenché, il **n'avait pas l'attribut de quarantaine.**
```bash
...
* Compresser l'application en un fichier tar.gz.
