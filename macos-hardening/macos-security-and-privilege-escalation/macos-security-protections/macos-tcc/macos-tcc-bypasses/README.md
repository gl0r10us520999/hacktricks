...
Можливо **помістити вікно над запитом TCC**, щоб змусити користувача **прийняти** його, не помітивши. Ви можете знайти PoC у [**TCC-ClickJacking**](https://github.com/breakpointHQ/TCC-ClickJacking)**.**
...
Більше того, можливо **видалити легітимний додаток з Dock і помістити фальшивий**, так що коли користувач натискає на фальшивий (який може використовувати той же значок), він може викликати легітимний, запитати дозволи TCC і виконати шкідливе ПЗ, змушуючи користувача вірити, що легітимний додаток запитав доступ.
...
Ця **`rename(a, b);`** поведінка вразлива до **Race Condition**, оскільки можливо помістити всередину папки `Automatically Add to Music.localized` підроблений **TCC.db** файл, а потім, коли нова папка (b) створюється, скопіювати файл, видалити його і вказати на **`~/Library/Application Support/com.apple.TCC`**/.

### SQLITE\_SQLLOG\_DIR - CVE-2023-32422

Якщо **`SQLITE_SQLLOG_DIR="path/folder"`**, це в основному означає, що **будь-яка відкрита база даних копіюється в цей шлях**. У цьому CVE цей контроль був зловжито для **запису** всередині **SQLite бази даних**, яка буде **відкрита процесом з FDA TCC базою даних**, а потім зловжити **`SQLITE_SQLLOG_DIR`** з **символічним посиланням у назві файлу**, так що коли ця база даних **відкрита**, користувач **TCC.db перезаписується** з відкритою.
...
Як root ви можете увімкнути цю службу, і **агент ARD матиме повний доступ до диска**, який потім може бути зловжито користувачем, щоб змусити його скопіювати нову **базу даних користувача TCC**.
...
Існують різні техніки для впровадження коду всередину процесу та зловживання його привілеями TCC:

{% content-ref url="../../../macos-proces-abuse/" %}
[macos-proces-abuse](../../../macos-proces-abuse/)
{% endcontent-ref %}

Більше того, найпоширеніше впровадження процесу для обходу TCC, яке було знайдено, відбувається через **плагіни (завантажити бібліотеку)**.\
...
### Плагіни абстрактного рівня пристрою (DAL)
