...
ユーザー名前空間は、**ユーザーおよびグループIDマッピングの隔離を提供するLinuxカーネルの機能**です。これにより、各ユーザー名前空間は**独自のユーザーおよびグループIDのセットを持つことができます**。この隔離により、異なるユーザー名前空間で実行されているプロセスは、同じユーザーおよびグループIDを数値的に共有していても、**異なる特権と所有権を持つことができます**。

ユーザー名前空間は特にコンテナ化において有用であり、各コンテナは独自のユーザーおよびグループIDの独立したセットを持つべきであり、これによりコンテナとホストシステム間のセキュリティと隔離が向上します。

### 仕組み：

1. 新しいユーザー名前空間が作成されると、**ユーザーおよびグループIDマッピングの空のセットから始まります**。これは、新しいユーザー名前空間で実行されるプロセスが**名前空間の外で特権を持たないことを意味します**。
2. 新しい名前空間のユーザーおよびグループIDと親（またはホスト）名前空間のIDとの間にIDマッピングを確立できます。これにより、**新しい名前空間のプロセスは、親名前空間のユーザーおよびグループIDに対応する特権と所有権を持つことができます**。ただし、IDマッピングは特定の範囲やIDのサブセットに制限でき、これにより新しい名前空間のプロセスに付与される特権を細かく制御できます。
...
例えば、ユーザー名前空間内で`CAP_SYS_ADMIN`権限を持っている場合、ファイルシステムのマウントなど、この権限を通常必要とする操作を実行できますが、あなたのユーザー名前空間のコンテキスト内でのみです。この権限を使って実行する操作は、ホストシステムや他の名前空間には影響しません。
