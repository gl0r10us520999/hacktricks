...
3. У межах простору користувача **процеси можуть мати повні привілеї root (UID 0) для операцій всередині простору**, при цьому все ще маючи обмежені привілеї поза межами простору. Це дозволяє **контейнерам працювати з можливостями, подібними до root, у своєму власному просторі без повних привілеїв root на хост-системі**.
