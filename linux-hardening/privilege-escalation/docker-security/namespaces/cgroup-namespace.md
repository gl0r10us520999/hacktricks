...
3. Cgroup namespaces не забезпечують безпосередньої ізоляції ресурсів; **вони лише забезпечують ізоляцію вигляду ієрархії cgroup**. **Контроль ресурсів та ізоляція все ще забезпечуються підсистемами cgroup** (наприклад, cpu, пам'ять тощо).
...
### Увійти всередину простору імен CGroup
```bash
nsenter -C TARGET_PID --pid /bin/bash
```
Також, ви можете **входити в інше просторове ім'я процесу лише якщо ви root**. І ви **не можете** **входити** в інше просторове ім'я **без дескриптора**, що вказує на нього (наприклад, `/proc/self/ns/cgroup`).
