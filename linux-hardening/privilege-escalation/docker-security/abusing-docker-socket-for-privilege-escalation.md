...
Jy kan ook 'n **mount misbruik om privileges te verhoog** binne die container.
...
Let ook daarop dat as jy **`/etc`** of enige ander gids **wat konfigurasie lêers bevat**, kan mount, jy dit mag verander vanuit die docker container as root om dit te **misbruik in die host** en privileges te verhoog (miskien deur `/etc/shadow` te wysig).
{% endhint %}

### Ontsnapping uit die container

* **`--privileged`** -> Met hierdie vlag [verwyder jy al die isolasie van die container](docker-privileged.md#what-affects). Kyk tegnieke om [uit geprivilegieerde containers as root te ontsnap](docker-breakout-privilege-escalation/#automatic-enumeration-and-escape).
...
Op hierdie bladsy het ons maniere bespreek om privileges te verhoog met behulp van docker vlae, jy kan **maniere vind om hierdie metodes te misbruik met die curl** opdrag op die bladsy:
