# Groupes Intéressants - Privesc Linux
...
```
ou Lors d'une nouvelle connexion de session ssh.
```bash
$ pspy64
...
```
Enfin, si vous n'aimez aucune des suggestions précédentes, ou qu'elles ne fonctionnent pas pour une raison quelconque (pare-feu de l'API docker ?), vous pourriez toujours essayer de **lancer un conteneur privilégié et d'en échapper** comme expliqué ici :
