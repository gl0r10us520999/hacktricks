...
`Attacks -> Web Drive-by -> Scripted Web Delivery (S)` 这将生成一个脚本/可执行文件，以下载来自cobalt strike的信标，格式包括：bitsadmin、exe、powershell和python
...
## 使用make_token会生成事件4624：账户成功登录。此事件在Windows域中非常常见，但可以通过过滤登录类型来缩小范围。如上所述，它使用LOGON32_LOGON_NEW_CREDENTIALS，即类型9。
...
## 从API文档中我们知道，这种登录类型“允许调用者克隆其当前令牌”。这就是为什么信标输出显示冒充了&#x3C;current_username> - 它正在冒充我们自己的克隆令牌。
ls \\computer_name\c$ # 尝试使用生成的令牌访问计算机中的C$
...
## 如果没有/run，mimikatz会生成一个cmd.exe，如果您以具有桌面的用户身份运行，他将看到shell（如果您以SYSTEM身份运行，您就可以继续）
...
使用[ThreatCheck](https://github.com/rasta-mouse/ThreatCheck)与生成的后门（或仅使用编译的模板），您可以找到触发防御者的原因。通常是一个字符串。因此，您可以修改生成后门的代码，以便该字符串不出现在最终的二进制文件中。
