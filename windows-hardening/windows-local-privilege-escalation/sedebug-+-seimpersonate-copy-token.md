...
이 경우, 이 코드는 컴파일되어 **Windows 서비스 바이너리**로 사용되어 작동 여부를 확인할 수 있습니다.\
그러나 **권한 상승이 발생하는 코드의 주요 부분**은 **`Exploit`** **함수** 내부에 있습니다.\
해당 함수 내부에서 **프로세스 _**lsass.exe**_**가 검색되고, 그 후 **토큰이 복사되며**, 마지막으로 그 토큰이 복사된 토큰의 모든 권한으로 새로운 _**cmd.exe**_를 생성하는 데 사용됩니다.
