...
Ayrıcalıkları yükseltmek için, en iyi şansımız, **bir ayrıcalıklı sürecin yüklemeye çalışacağı bir dll yazabilmektir** bazı **yerlerde aranacak**. Bu nedenle, **orijinal dll'in** bulunduğu dizinden önce **dll'in arandığı** bir **dizine** yazabileceğiz (garip bir durum), ya da **dll'in aranacağı** bir dizine yazabileceğiz ve orijinal **dll herhangi bir dizinde mevcut değildir**.
...
* Eğer bir **DLL bağımlılıklara sahipse**, bu bağımlı DLL'ler için arama, yalnızca **modül adlarıyla** belirtildiği gibi yapılır, ilk DLL'in tam yol ile tanımlanıp tanımlanmadığına bakılmaksızın.

### Escalating Privileges

**Requirements**:

* **Farklı ayrıcalıklar** altında çalışan veya çalışacak bir süreci tanımlayın (yatay veya yan hareket), **bir DLL'den yoksun**.
* **DLL'nin aranacağı** herhangi bir **dizinde** **yazma erişiminin** mevcut olduğundan emin olun. Bu konum, yürütülebilir dosyanın dizini veya sistem yolundaki bir dizin olabilir.
...
Ayrıca bir çalıştırılabilir dosyanın içe aktarımlarını ve bir dll'nin dışa aktarımlarını kontrol edebilirsiniz:
```c
dumpbin /imports C:\path\Tools\putty\Putty.exe
dumpbin /export /path/file.dll
```
Tam yetkileri artırmak için **Dll Hijacking'i kötüye kullanma** hakkında tam bir rehber için **System Path klasöründe yazma izinlerinizin olup olmadığını kontrol etmek için**:
...
Temelde bir **Dll proxy**, yüklendiğinde **kötü niyetli kodunuzu çalıştırabilen** ama aynı zamanda **gerçek kütüphaneye yapılan tüm çağrıları ileterek** **görünmesini** ve **çalışmasını** sağlayan bir Dll'dir.
...
Dikkat edin ki, birkaç durumda derlediğiniz Dll'nin, kurban süreci tarafından yüklenecek **birkaç işlevi dışa aktarması** gerekir; eğer bu işlevler mevcut değilse, **ikili yüklenemeyecek** ve **sömürü başarısız olacaktır**.
```c
// Tested in Win10
