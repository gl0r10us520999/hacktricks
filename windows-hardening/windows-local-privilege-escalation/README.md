...
**Credential Guard** 是在 **Windows 10** 中引入的。它的目的是保护存储在设备上的凭据，防止像 pass-the-hash 攻击这样的威胁。| [**关于 Credentials Guard 的更多信息。**](../stealing-credentials/credentials-protections.md#credential-guard)
```bash
...
首先，列出进程 **检查进程命令行中的密码**。\
检查您是否可以 **覆盖某个正在运行的二进制文件**，或者您是否具有二进制文件夹的写入权限，以利用可能的 [**DLL 劫持攻击**](dll-hijacking/):
```bash
...
如果您对注册表拥有此权限，这意味着**您可以从此注册表创建子注册表**。在 Windows 服务的情况下，这**足以执行任意代码：**
...
安装程序是**以SYSTEM权限运行的**，许多程序容易受到**DLL侧载攻击（信息来自** [**https://github.com/enjoiz/Privesc**](https://github.com/enjoiz/Privesc)**）。**
```bash
...
[阅读这篇**其他文章以获取更完整的解释，了解如何测试和滥用具有不同权限级别（不仅仅是完全访问权限）继承的进程和线程的更多打开句柄**](http://dronesec.pw/blog/2019/08/22/exploiting-leaked-process-and-thread-handles/).
...
这使得可以在同一漏洞下同时提升权限并绕过 UAC。此外，无需安装任何东西，过程中使用的二进制文件由 Microsoft 签名并发布。
...
[~~**Beroot**~~](https://github.com/AlessandroZ/BeRoot) **\~\~**\~\~ -- 检查错误配置（在 github 上有可执行的预编译版本）。不推荐。它在 Win10 上效果不好。\
