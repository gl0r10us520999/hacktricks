...
**Wmic** kann verwendet werden, um Programme beim **Start** auszuführen. Sieh dir an, welche Binaries so programmiert sind, dass sie beim Start ausgeführt werden:
```bash
...
Registrierungsschlüssel, die als **Run** und **RunOnce** bekannt sind, sind so konzipiert, dass sie Programme automatisch ausführen, jedes Mal, wenn sich ein Benutzer im System anmeldet. Die Befehlszeile, die als Datenwert eines Schlüssels zugewiesen ist, ist auf 260 Zeichen oder weniger beschränkt.
...
{% hint style="info" %}
**Exploit 2**: Wenn Sie eine der angegebenen Binärdateien in einer der Registrierungen innerhalb von **HKLM** überschreiben können, können Sie diese Binärdatei mit einem Hintertürchen modifizieren, wenn sich ein anderer Benutzer anmeldet, und die Berechtigungen erhöhen.
{% endhint %}
```bash
#CMD
...
{% hint style="info" %}
Wenn Sie einen beliebigen \[User] Shell Folder unter **HKLM** überschreiben können, sind Sie in der Lage, ihn auf einen von Ihnen kontrollierten Ordner zu verweisen und eine Hintertür zu platzieren, die jedes Mal ausgeführt wird, wenn ein Benutzer sich im System anmeldet, wodurch die Berechtigungen erhöht werden.
{% endhint %}
```bash
...
Um die **`StubPath`**-Konfigurationen über die Active Setup-Komponenten zu überprüfen, können diese Befehle verwendet werden:
```bash
