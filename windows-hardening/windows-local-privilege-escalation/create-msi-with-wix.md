...
MSI 설치 프로그램의 생성은 wixtools를 사용하여 수행되며, 특히 [wixtools](http://wixtoolset.org)가 활용됩니다. 대체 MSI 빌더가 시도되었으나, 이 특정 경우에는 성공하지 못했다는 점은 언급할 가치가 있습니다.
...
```
패키지 요소에는 설치 프로그램의 버전을 지정하는 InstallerVersion 및 패키지가 압축되었는지 여부를 나타내는 Compressed와 같은 속성이 포함되어 있다는 점에 유의하는 것이 중요합니다.

생성 과정에는 msi.xml에서 wixobject를 생성하기 위해 wixtools의 도구인 candle.exe를 사용하는 것이 포함됩니다. 다음 명령을 실행해야 합니다:
```
candle.exe -out C:\tem\wix C:\tmp\Ethereal\msi.xml
```
추가로, 게시물에 명령과 그 출력이 포함된 이미지가 제공된다는 점은 언급할 가치가 있습니다. 시각적 안내를 위해 참조할 수 있습니다.
