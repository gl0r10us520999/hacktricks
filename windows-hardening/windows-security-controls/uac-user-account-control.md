...
您可以使用 Sysinternals 的工具 _**sigcheck.exe**_ 检查二进制文件的 _**清单**_。您可以使用 _Process Explorer_ 或 _Process Monitor_（来自 Sysinternals）查看 **进程的完整性级别**。
...
如果你有一个属于管理员组的用户的 shell，你可以 **通过 SMB 挂载 C$** 共享到一个新的磁盘上，你将 **访问到文件系统中的所有内容**（甚至是管理员的主文件夹）。
...
2. 使用 procmon 查找可能受到 **DLL 劫持** 影响的 "**NAME NOT FOUND**" 事件。
