...
**일반 대화상자**는 **파일 저장**, **파일 열기**, 글꼴 선택, 색상 선택 등의 옵션입니다. 대부분은 **전체 탐색기 기능을 제공합니다**. 즉, 이러한 옵션에 접근할 수 있다면 탐색기 기능에 접근할 수 있습니다:
...
이미지를 클릭하면 왼쪽 상단에 나타나는 도구 모음입니다. 저장, 인쇄, 메일, 탐색기에서 "내 사진" 열기를 할 수 있습니다. Kiosk는 Internet Explorer를 사용해야 합니다.
...
* 다섯 손가락으로 화면을 집어넣기 / 홈 버튼 터치 / 화면 하단에서 한 손가락으로 빠르게 위로 스와이프: 홈에 접근합니다.
...
* 화면 왼쪽에서 한 손가락으로 1-2인치 스와이프: 오늘 보기 확인합니다.
* 화면 중앙에서 한 손가락으로 빠르게 오른쪽 또는 왼쪽으로 스와이프: 다음/이전 앱으로 변경합니다.
* 오른쪽 상단 모서리의 전원/슬립 버튼을 누르고 **전원 끄기** 슬라이더를 오른쪽으로 끝까지 이동: 전원 끕니다.
* 오른쪽 상단 모서리의 전원/슬립 버튼과 홈 버튼을 몇 초 동안 누르기: 강제로 하드 전원 끄기.
* 오른쪽 상단 모서리의 전원/슬립 버튼과 홈 버튼을 빠르게 누르기: 화면 왼쪽 하단에 팝업되는 스크린샷을 찍습니다. 두 버튼을 동시에 아주 짧게 누르세요. 몇 초 동안 누르면 하드 전원 꺼짐이 수행됩니다.
...
| ⌘\~                                                | 마지막 앱으로 이동                                    |
| ⌘⇧3 (Command-Shift-3)                              | 스크린샷 (저장하거나 작업을 수행하기 위해 왼쪽 하단에 떠 있습니다) |
| ⌘⇧4                                                | 스크린샷을 찍고 편집기로 열기                        |
