...
* **`Windows Executable (S)`** は **ステージレス** .exe、.dll またはサービス .exe 用（ステージレスの方がステージドよりも良い、IoCが少ない）
...
```
`dist-pipe\artifact.cna`という攻撃的なスクリプトを読み込むのを忘れないでください。これにより、Cobalt Strikeが使用したいディスク上のリソースを指定し、読み込まれたリソースではなくなります。
