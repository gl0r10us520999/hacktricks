...
要发现手动安装或在这些软件包管理器之外安装的软件，探索目录如 _**`/usr/local`**_、_**`/opt`**_、_**`/usr/sbin`**_、_**`/usr/bin`**_、_**`/bin`**_ 和 _**`/sbin`**_。将目录列表与特定于系统的命令结合使用，以识别与已知软件包无关的可执行文件，从而增强您对所有已安装程序的搜索。
```bash
...
此外，检查像 _**/etc/sudoers**_ 和 _**/etc/groups**_ 这样的文件，寻找意外授予用户的特权。\
...
````
{% hint style="info" %}
注意，**攻击者**可以**修改****时间**以使**文件看起来**是**合法的**，但他**无法**修改**inode**。如果你发现一个**文件**显示它的创建和修改时间与同一文件夹中其他文件的**时间相同**，但**inode**却**意外地更大**，那么该**文件的时间戳被修改了**。
{% endhint %}
