...
这可能意味着您**没有正确添加**魔法数字，或者您没有**使用**正确的魔法数字，因此请**确保使用正确的**（或尝试一个新的）。
...
如果您无法按照之前的步骤提取 python 的“原始”代码，那么您可以尝试 **extract** **assembly**（但 **它不是很描述性**，所以 **尝试** 再次提取 **原始代码**）。在 [这里](https://bits.theorem.co/protecting-a-python-codebase/) 我找到了一段非常简单的代码来 **disassemble** _.pyc_ 二进制文件（祝您理解代码流程好运）。如果 _.pyc_ 是来自 python2，请使用 python2：
```bash
>>> import dis
>>> import marshal
>>> import struct
