...
'550d0d0a'
```
Liczba **magic** w tym przypadku dla python3.8 to **`0x550d0d0a`**, więc aby naprawić ten błąd, musisz **dodać** na **początku** **.pyc file** następujące bajty: `0x0d550a0d000000000000000000000000`
...
```
### Błąd: Decompiling generic errors

**Inne błędy** takie jak: `class 'AssertionError'>; co_code powinno być jednym z typów (<class 'str'>, <class 'bytes'>, <class 'list'>, <class 'tuple'>); jest typu <class 'NoneType'>` mogą się pojawić.
...
Sprawdź dokumentację poprzednich błędów.
...
Jeśli nie udało ci się wyodrębnić "oryginalnego" kodu python zgodnie z poprzednimi krokami, możesz spróbować **wyodrębnić** **assembly** (ale **nie jest to zbyt opisowe**, więc **spróbuj** ponownie wyodrębnić **oryginalny** kod). W [tutaj](https://bits.theorem.co/protecting-a-python-codebase/) znalazłem bardzo prosty kod do **dezaktywacji** binarnego pliku _.pyc_ (powodzenia w zrozumieniu przepływu kodu). Jeśli _.pyc_ jest z python2, użyj python2:
```bash
>>> import dis
>>> import marshal
>>> import struct
