...
list(xrange()) == range() --> En python3, range est l'xrange de python2 (ce n'est pas une liste mais un générateur)\
...
Si vous faites 2/3, cela renvoie 1 car vous divisez deux entiers (ints). Si vous voulez des décimales, vous devez diviser des flottants (2.0/3.0).\
i >= j\
i <= j\
i == j\
i != j\
...
(lambda x,y: x+y)(5,3) = 8 --> Utilisez lambda comme simple **function**\
...
from itertools import **combinations\_with\_replacement** --> Génère toutes les combinaisons possibles à partir du caractère suivant (par exemple, le 3ème est mélangé à partir du 3ème mais pas avec le 2ème ou le premier)\
