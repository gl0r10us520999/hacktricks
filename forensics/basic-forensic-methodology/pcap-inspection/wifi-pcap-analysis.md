...
```
Per esempio, recupererà la passphrase WPA che protegge un PSK (pre shared-key), che sarà necessaria per decrittare il traffico in seguito.

# Dati nei Beacon / Canale Laterale

Se sospetti che **i dati vengano trapelati all'interno dei beacon di una rete Wifi**, puoi controllare i beacon della rete utilizzando un filtro come il seguente: `wlan contains <NAMEofNETWORK>`, o `wlan.ssid == "NAMEofNETWORK"` cerca all'interno dei pacchetti filtrati stringhe sospette.
