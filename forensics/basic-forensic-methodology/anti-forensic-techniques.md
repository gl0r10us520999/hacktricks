...
Możliwe jest znalezienie znaczników czasowych w MFT w atrybutach `$STANDARD_INFORMATION` __ i __ `$FILE_NAME`.
...
* MTIME: Modyfikacja rejestru MFT
...
Kiedy folder jest otwierany z woluminu NTFS na serwerze Windows NT, system zajmuje czas na **aktualizację pola znacznika czasowego w każdym wymienionym folderze**, nazywanego czasem ostatniego dostępu. Na mocno używanym woluminie NTFS może to wpłynąć na wydajność.
...
Możesz również użyć narzędzia [**USBDeview**](https://www.nirsoft.net/utils/usb\_devices\_view.html), aby upewnić się, że je usunąłeś (i aby je usunąć).
