...
```
Em este caso, as informações serão salvas dentro de um arquivo CSV.
...
(_Note que os timestamps fornecidos pelo JumplistExplorer estão relacionados ao arquivo jumplist em si_)
...
- Windows Vista e versões posteriores fazem backup dos arquivos de registro `HKEY_LOCAL_MACHINE` em `%Windir%\System32\Config\RegBack\`.
...
O prefetch do Windows consiste em criar **caches dos programas executados** para poder carregá-los mais rápido. Esses caches são criados como arquivos `.pf` dentro do caminho: `C:\Windows\Prefetch`. Há um limite de 128 arquivos em XP/VISTA/WIN7 e 1024 arquivos em Win8/Win10.
...
Este arquivo é notável por armazenar registros de processos executados recentemente, incluindo os caminhos para os arquivos executáveis e seus hashes SHA1. Esta informação é inestimável para rastrear a atividade de aplicações em um sistema.
