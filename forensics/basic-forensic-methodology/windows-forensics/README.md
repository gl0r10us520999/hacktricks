...
Montiranjem forenzičke slike sa **ArsenalImageMounter**, alat [**ShadowCopyView**](https://www.nirsoft.net/utils/shadow\_copy\_view.html) može se koristiti za inspekciju shadow copy i čak **ekstrakciju datoteka** iz rezervnih kopija shadow copy.
...
Možete pronaći automatski sačuvane datoteke u Office-u na: `C:\Usuarios\\AppData\Roaming\Microsoft{Excel|Word|Powerpoint}\`
...
Takođe, **datum kreiranja link** datoteke je prvi **put** kada je originalna datoteka **prvi put** **korišćena**, a **datum** **modifikacije** link datoteke je **poslednji** **put** kada je originalna datoteka korišćena.

Za inspekciju ovih datoteka možete koristiti [**LinkParser**](http://4discovery.com/our-tools/).
...
(_Napomena da su vremenske oznake koje pruža JumplistExplorer povezane sa samom datotekom jumplist_)
...
Proverite datoteku `C:\Windows\inf\setupapi.dev.log` da dobijete vremenske oznake o tome kada je USB konekcija napravljena (pretražujte za `Section start`).
...
Datoteka `C:\Windows\Prefetch\Layout.ini` sadrži **imena foldera datoteka koje su prefetched**. Ova datoteka sadrži **informacije o broju izvršenja**, **datumima** izvršenja i **datotekama** **otvorenim** od strane programa.
...
**Superprefetch** ima isti cilj kao i prefetch, **brže učitavanje programa** predviđanjem šta će biti učitano sledeće. Međutim, ne zamenjuje uslugu prefetch.\
...
Ovi podaci se čuvaju unutar registra na specifičnim lokacijama u zavisnosti od verzije operativnog sistema:
...
- **Daljinsko interaktivno (10)**: Prijava putem daljinske radne površine ili terminalskih servisa.
...
Korisni sistemski EventID-ovi za praćenje USB uređaja uključuju 20001/20003/10000 za inicijalnu upotrebu, 10100 za ažuriranja drajvera i EventID 112 iz DeviceSetupManager-a za vremenske oznake umetanja.
