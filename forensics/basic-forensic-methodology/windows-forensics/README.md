...
Timeline - це характеристика Windows, яка надає **хронологічну історію** відвіданих веб-сторінок, редагованих документів та виконаних програм.
...
Ви можете знайти автоматично збережені файли Office в: `C:\Usuarios\\AppData\Roaming\Microsoft{Excel|Word|Powerpoint}\`
...
Можливо, ідентифікувати, що USB-пристрій був використаний завдяки створенню:
...
- **pnpclean.dll**: Цей DLL відповідає за фактичний процес очищення.
...
У `SAM\Domains\Account\Users` ви можете отримати ім'я користувача, RID, останній вхід, останній невдалий вхід, лічильник входу, політику паролів та коли обліковий запис був створений. Щоб отримати **хеші**, вам також **потрібен** файл/хвіст **SYSTEM**.
...
Prefetch Windows складається зі створення **кешів виконаних програм**, щоб мати можливість завантажувати їх швидше. Ці кеші створюються як `.pf` файли всередині шляху: `C:\Windows\Prefetch`. Існує обмеження в 128 файлів у XP/VISTA/WIN7 та 1024 файлів у Win8/Win10.

Ім'я файлу створюється як `{program_name}-{hash}.pf` (хеш базується на шляху та аргументах виконуваного файлу). У W10 ці файли стиснуті. Зверніть увагу, що сама наявність файлу вказує на те, що **програма була виконана** в якийсь момент.
...
У цих базах даних ви можете знайти **ім'я** **програми**, **кількість** **виконань**, **файли** **відкриті**, **обсяг** **доступу**, **повний** **шлях**, **часові рамки** та **мітки часу**.
...
Файл **Amcache.hve** є по суті гніздом реєстру, яке фіксує деталі про програми, що виконувалися на системі. Зазвичай його можна знайти за адресою `C:\Windows\AppCompat\Programas\Amcache.hve`.
...
- **0xC000006A**: Правильне ім'я користувача, але неправильний пароль - Можлива спроба вгадати пароль або атака методом грубої сили.
...
EventID безпеки 1102 сигналізує про видалення журналів, критична подія для судово-медичного аналізу.
