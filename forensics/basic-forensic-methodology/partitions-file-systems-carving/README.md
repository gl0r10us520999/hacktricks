...
Die **minimale** Einheit einer Festplatte ist der **Sektor** (normalerweise aus 512B zusammengesetzt). Daher muss die Größe jeder Partition ein Vielfaches dieser Größe sein.
...
Er erlaubt bis zu **4 Partitionen** (maximal **nur 1** kann aktiv/**bootfähig** sein). Wenn du jedoch mehr Partitionen benötigst, kannst du **erweiterte Partitionen** verwenden. Das **letzte Byte** dieses ersten Sektors ist die Bootrecord-Signatur **0x55AA**. Nur eine Partition kann als aktiv markiert werden.\
...
Von den **Bytes 440 bis 443** des MBR kannst du die **Windows-Disk-Signatur** finden (wenn Windows verwendet wird). Der logische Laufwerksbuchstabe der Festplatte hängt von der Windows-Disk-Signatur ab. Das Ändern dieser Signatur könnte verhindern, dass Windows bootet (Tool: [**Active Disk Editor**](https://www.disk-editor.org/index.html)**)**.
...
Um ein MBR in Linux zu mounten, musst du zuerst den Start-Offset ermitteln (du kannst `fdisk` und den `p`-Befehl verwenden)
...
Wenn es sich um eine **GPT-Tabelle anstelle eines MBR** gehandelt hätte, sollte die Signatur _EFI PART_ im **Sektor 1** erscheinen (der im vorherigen Bild leer ist).
...
Einige Dateien enthalten Metadaten. Diese Informationen beziehen sich auf den Inhalt der Datei, die für einen Analysten manchmal interessant sein könnten, da sie je nach Dateityp Informationen wie enthalten kann:
...
Sie werden feststellen, dass selbst bei dieser Aktion möglicherweise **andere Teile, in denen die Existenz der Datei weiterhin protokolliert ist**, vorhanden sind, und das ist wahr, und ein Teil der Arbeit eines forensischen Fachmanns besteht darin, diese zu finden.
