...
**公钥证书**是用于加密的数字身份证明，用于证明某人拥有公钥。它包括密钥的详细信息、所有者的身份（主题）以及来自受信任机构（颁发者）的数字签名。如果软件信任颁发者并且签名有效，则可以与密钥所有者进行安全通信。
...
**OCSP**（**RFC 2560**）涉及客户端和响应者共同检查数字公钥证书是否已被撤销，而无需下载完整的**CRL**。这种方法比传统的**CRL**更高效，后者提供被撤销证书序列号的列表，但需要下载一个可能很大的文件。CRL最多可以包含512个条目。更多细节可在[这里](https://www.arubanetworks.com/techdocs/ArubaOS%206\_3\_1\_Web\_Help/Content/ArubaFrameStyles/CertRevocation/About\_OCSP\_and\_CRL.htm)找到。
