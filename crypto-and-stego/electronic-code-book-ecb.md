...
Dans de nombreuses bases de données, il est identique de rechercher `WHERE username='admin';` ou `WHERE username='admin    ';` _(Remarque : les espaces supplémentaires)_
...
Le cookie de cet utilisateur sera composé de 3 blocs : les 2 premiers sont les blocs du nom d'utilisateur + délimiteur et le troisième celui du mot de passe (qui fait semblant d'être le nom d'utilisateur) : `username       |admin   `
