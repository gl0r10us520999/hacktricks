...
- **La preuve de travail (PoW)** repose sur la puissance de calcul pour la vérification des transactions.
- **La preuve d'enjeu (PoS)** exige que les validateurs détiennent un certain nombre de tokens, réduisant la consommation d'énergie par rapport à la PoW.
...
Vise à améliorer l'évolutivité de Bitcoin en permettant plusieurs transactions au sein d'un canal, ne diffusant l'état final à la blockchain.

## Préoccupations de confidentialité de Bitcoin

Les attaques de confidentialité, telles que **l'hypothèse de propriété d'entrée commune** et **la détection d'adresse de changement UTXO**, exploitent les modèles de transaction. Des stratégies comme **les mixeurs** et **CoinJoin** améliorent l'anonymat en obscurcissant les liens de transaction entre les utilisateurs.
...
Les transactions d'exemple qui ont pu utiliser CoinJoin incluent `402d3e1df685d1fdf82f36b220079c1bf44db227df2d676625ebcbee3f6cb22a` et `85378815f6ee170aa8c26694ee2df42b99cff7fa9357f073c1192fff1f540238`.
...
Une variante de CoinJoin, **PayJoin** (ou P2EP), déguisent la transaction entre deux parties (par exemple, un client et un commerçant) comme une transaction ordinaire, sans les caractéristiques distinctives des sorties égales de CoinJoin. Cela rend extrêmement difficile la détection et pourrait invalider l'heuristique de propriété d'entrée commune utilisée par les entités de surveillance des transactions.
```plaintext
2 btc --> 3 btc
5 btc     4 btc
```
