...
```
* Tensors wat van Python heelgetalle geskep is, is van tipe `torch.int64`.
* Tensors wat van Python floats geskep is, is van tipe `torch.float32`.
...
```
*   **Transposisie van Tensors**: Gebruik `.T` om 'n 2D tensor te transposieer.
...
* Gradiënte word opgetel in die `.grad` attribuut van elke parameter tensor.
