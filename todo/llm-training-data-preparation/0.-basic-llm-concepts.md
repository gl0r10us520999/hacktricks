...
```
*   **Transposering van Tensors**: Gebruik `.T` om 'n 2D tensor te transposieer.
...
In AD, word berekeninge verteenwoordig as knope in 'n **berekening grafiek**, waar elke knoop ooreenstem met 'n operasie of 'n veranderlike. Deur hierdie grafiek te traverseer, kan ons afgeleides doeltreffend bereken.
...
* **Vorentoe Pass:** Bereken die uitsette van die netwerk.
* **Agtertoe Pass:** `loss.backward()` bereken die gradiënte van die verlies ten opsigte van alle parameters.
* **Parameter Opdatering:** `optimizer.step()` werk die parameters op gebaseer op die berekende gradiënte.

### **5. Verstaan Agtertoe Pass**

Tydens die agtertoe pass:
