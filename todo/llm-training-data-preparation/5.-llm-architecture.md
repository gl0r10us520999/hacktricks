...
Mfano wa usanifu wa LLM kutoka [https://github.com/rasbt/LLMs-from-scratch/blob/main/ch04/01\_main-chapter-code/ch04.ipynb](https://github.com/rasbt/LLMs-from-scratch/blob/main/ch04/01_main-chapter-code/ch04.ipynb):
...
1. **Input (Maandishi Yaliyotolewa Token)**: Mchakato huanza na maandiko yaliyotolewa token, ambayo yanabadilishwa kuwa uwakilishi wa nambari.
2. **Tabaka la Token Embedding na Positional Embedding**: Maandishi yaliyotolewa token yanapita kupitia **tabaka la token embedding** na **tabaka la positional embedding**, ambayo yanakamata nafasi ya tokens katika mfuatano, muhimu kwa kuelewa mpangilio wa maneno.
...
* **Feed Forward Layer**: Inawajibika kwa kuchakata habari kutoka kwenye tabaka la attention na kufanya utabiri kuhusu token inayofuata.
...
5. **Lengo**: Lengo ni kuchukua embeddings hizi na kuzibadilisha tena kuwa maandiko. Kwa hakika, safu ya mwisho ya matokeo inatumika kuzalisha neno linalofuata, linalowakilishwa kama "forward" katika mchoro huu.
...
* **Kuamsha Kunyumbulika:** Tofauti na ReLU, ambayo inafanya kuwa sifuri kwa pembejeo hasi, GELU inachora kwa urahisi pembejeo kuwa matokeo, ikiruhusu thamani ndogo, zisizo sifuri kwa pembejeo hasi.
...
* **Hesabu Variance (`var`):** Hesabu variance ya `x` kati ya dimension ya embedding, pia ikihifadhi dimension. Kigezo `unbiased=False` kinahakikisha kuwa variance inahesabiwa kwa kutumia mhesabu wa biased (kugawanya na `N` badala ya `N-1`), ambayo ni sahihi wakati wa kurekebisha juu ya vipengele badala ya sampuli.
...
{% hint style="info" %}
Lengo ni kuhakikisha mean ya 0 na variance ya 1 kati ya vipimo vyote vya token sawa. Lengo la hili ni **kuimarisha mafunzo ya mitandao ya neva ya kina** kwa kupunguza mabadiliko ya ndani ya covariate, ambayo inahusisha mabadiliko katika usambazaji wa uhamasishaji wa mtandao kutokana na kubadilishwa kwa vigezo wakati wa mafunzo.
{% endhint %}
...
* **Kuacha:** Unatumika baada ya tabaka za umakini na feedforward kwa ajili ya urekebishaji.
...
* **Kuacha (`drop_shortcut`):** Tumia kuacha kwa urekebishaji.
...
* **Kuacha (`drop_shortcut`):** Tumia kuacha.
* **Ongeza Ziada (`x + shortcut`):** Changanya na ingizo kutoka kwa njia ya kwanza ya ziada.

{% hint style="info" %}
Bloku ya transformer inakusanya mitandao yote pamoja na kutumia **urekebishaji** na **kuacha** ili kuboresha utulivu wa mafunzo na matokeo.\
Kumbuka jinsi kuacha kunavyofanywa baada ya matumizi ya kila mtandao wakati urekebishaji unatumika kabla.
...
_Maumbo yameongezwa kama maelezo ili kuelewa vyema maumbo ya matrices:_
```python
# From https://github.com/rasbt/LLMs-from-scratch/tree/main/ch04
class GPTModel(nn.Module):
def __init__(self, cfg):
...
* **Transformer Blocks (`trf_blocks`):** Msururu wa `n_layers` transformer blocks ili kushughulikia embeddings.
...
{% hint style="info" %}
Lengo la darasa hili ni kutumia mitandao mingine yote iliyotajwa ili **kutabiri token inayofuata katika mfululizo**, ambayo ni muhimu kwa kazi kama vile uzalishaji wa maandiko.
