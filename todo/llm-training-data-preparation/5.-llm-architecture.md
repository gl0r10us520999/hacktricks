...
1. **Input (Tokenized Text)**: Η διαδικασία ξεκινά με το κείμενο που έχει διαχωριστεί σε tokens, το οποίο μετατρέπεται σε αριθμητικές αναπαραστάσεις.
2. **Token Embedding and Positional Embedding Layer**: Το διαχωρισμένο κείμενο περνά μέσα από ένα **token embedding** layer και ένα **positional embedding layer**, το οποίο καταγράφει τη θέση των tokens σε μια ακολουθία, κρίσιμο για την κατανόηση της σειράς των λέξεων.
