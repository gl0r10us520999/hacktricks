...
또한, 훈련 중에 **인과 마스크**가 적용되어 나중의 토큰이 특정 토큰과의 관계를 찾을 때 고려되지 않으며, **드롭아웃**도 적용되어 **과적합을 방지**합니다.
{% endhint %}
...
{% hint style="info" %}
목표는 동일한 토큰의 모든 차원에서 평균이 0이고 분산이 1이 되도록 하는 것입니다. 이는 **딥 뉴럴 네트워크의 훈련을 안정화**하기 위한 것으로, 훈련 중 매개변수 업데이트로 인해 네트워크 활성화의 분포가 변화하는 내부 공변량 이동을 줄이는 것을 의미합니다.
{% endhint %}
...
또한, **네트워크의 출력을 입력과 더하는** 단축을 사용합니다. 이는 초기 레이어가 마지막 레이어만큼 "많이" 기여하도록 하여 소실 그래디언트 문제를 방지하는 데 도움이 됩니다.
{% endhint %}
...
또한, **출력** 이전에 **정규화** 레이어가 추가되고, 마지막에 적절한 차원의 결과를 얻기 위해 최종 선형 레이어가 적용됩니다. 각 최종 벡터가 사용된 어휘의 크기를 가지는 이유는 어휘 내 가능한 각 토큰에 대한 확률을 얻으려 하기 때문입니다.
{% endhint %}
...
모델이 이전과 같은 다음 토큰을 예측하는 경우, 출력에서 마지막 토큰 값을 가져오기만 하면 됩니다(예측된 토큰의 값이 될 것이므로). 이는 **어휘의 각 항목에 대한 값**이 될 것이며, 그런 다음 `softmax` 함수를 사용하여 차원을 확률로 정규화하여 합이 1이 되도록 하고, 가장 큰 항목의 인덱스를 가져오면, 이는 어휘 내의 단어 인덱스가 됩니다.
