...
2. **Tabaka za Token Embedding na Positional Embedding**: Maandishi yaliyotolewa token yanapita kupitia tabaka za **token embedding** na **positional embedding**, ambayo inashughulikia nafasi ya tokens katika mfuatano, muhimu kwa kuelewa mpangilio wa maneno.
...
_Umbo umeongezwa kama maoni ili kuelewa vyema umbo la matrices:_
```python
# From https://github.com/rasbt/LLMs-from-scratch/tree/main/ch04
class GPTModel(nn.Module):
def __init__(self, cfg):
