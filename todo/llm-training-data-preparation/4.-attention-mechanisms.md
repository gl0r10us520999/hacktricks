...
{% hint style="success" %}
Просто помножте значення кожного виміру запиту на відповідне значення кожного токена та додайте результати. Ви отримуєте 1 значення для кожної пари токенів.
{% endhint %}
...
Схоже на приклад з попереднього разу, але цього разу, замість використання значень вимірів токенів, ми використовуємо матрицю ключів токена (яка вже була розрахована за допомогою вимірів):. Отже, для кожного запиту `qi`​ та ключа `kj​`:
...
**Apply Softmax to Obtain Attention Weights:** Як у початковому прикладі, нормалізуйте всі значення, щоб їхня сума дорівнювала 1.&#x20;
...
```
## Розширення одно-головної уваги до багатоголової уваги

**Багатоголова увага** на практиці полягає в виконанні **декількох екземплярів** функції самоуваги, кожен з яких має **свої власні ваги**, тому розраховуються різні фінальні вектори.
