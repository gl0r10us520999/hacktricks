...
[**SigDigger** ](https://github.com/BatchDrake/SigDigger)는 GNU/Linux 및 macOS용 무료 디지털 신호 분석기로, 알려지지 않은 라디오 신호의 정보를 추출하도록 설계되었습니다. SoapySDR를 통해 다양한 SDR 장치를 지원하며, FSK, PSK 및 ASK 신호의 조정 가능한 복조, 아날로그 비디오 디코딩, 버스트 신호 분석 및 아날로그 음성 채널 청취(모두 실시간으로)를 허용합니다.
...
{% hint style="info" %}
PC가 신호를 캡처하지 못하는 경우 OpenGL을 비활성화하고 샘플 속도를 낮추어 보세요.
{% endhint %}
...
* SigDigger의 **튜너**는 **더 나은 신호를 캡처하는 데 도움**을 줍니다(하지만 신호를 저하시킬 수도 있습니다). 이상적으로는 0에서 시작하여 **신호 개선보다** **노이즈**가 **더 커질 때까지** **크기를 늘리세요**.

![](<../../.gitbook/assets/image (1099).png>)

### 라디오 채널과 동기화

[**SigDigger** ](https://github.com/BatchDrake/SigDigger)로 듣고 싶은 채널과 동기화하고 "Baseband audio preview" 옵션을 설정한 후, 전송되는 모든 정보를 얻기 위해 대역폭을 설정하고 튜너를 노이즈가 실제로 증가하기 시작하기 전의 수준으로 설정하세요:
...
신호를 확인할 때 정보를 저장하는 데 사용되는 방법을 알아내기 위한 다양한 방법이 있지만(아래에서 더 많은 방법을 찾으세요) 좋은 방법 중 하나는 IQ 그래프를 확인하는 것입니다.
...
* 정보가 위상이 변경된 사실에 숨겨져 있고 위상 자체에 숨겨져 있다면, 서로 다른 위상이 명확하게 구분되지 않을 것입니다.
...
[**SigDigger** ](https://github.com/BatchDrake/SigDigger)로 AM 정보를 확인하고 **봉투**를 보면 서로 다른 명확한 진폭 수준을 볼 수 있습니다. 사용된 신호는 AM으로 정보를 전송하는 펄스를 보내고 있으며, 하나의 펄스는 다음과 같이 보입니다:
...
많은 주파수를 발견하면 이는 FM이 아닐 가능성이 높으며, 아마도 신호 주파수는 채널 때문에 수정되었을 것입니다.
...
찾을 수 있는 가장 작은 기호를 선택하고(확실히 1개임을 확인) "선택 주파수"를 확인하세요. 이 경우 1.013kHz(즉, 1kHz)가 됩니다.

![](<../../.gitbook/assets/image (78).png>)

#### 기호 그룹으로

선택할 기호의 수를 지정할 수도 있으며, SigDigger는 1개의 기호의 주파수를 계산합니다(선택한 기호가 많을수록 더 좋습니다). 이 시나리오에서는 10개의 기호를 선택했으며 "선택 주파수"는 1.004 Khz입니다:

![](<../../.gitbook/assets/image (1008).png>)

### 비트 얻기

이것이 **AM 변조** 신호이고 **기호 속도**를 찾았으며(이 경우 어떤 것이 위로 가면 1이고 어떤 것이 아래로 가면 0임을 알고 있으므로), 신호에 인코딩된 **비트를 얻는 것이 매우 쉽습니다**. 따라서 정보를 포함한 신호를 선택하고 샘플링 및 결정을 구성한 후 샘플을 누르세요(확실히 **진폭**이 선택되어 있고, 발견된 **기호 속도**가 구성되어 있으며, **Gadner 클록 복구**가 선택되어 있는지 확인하세요):
...
* **Gadner 클록 복구**는 일반적으로 가장 좋은 옵션이지만, 여전히 대략적인 기호 속도를 지정해야 합니다.

샘플을 누르면 다음과 같은 화면이 나타납니다:
...
신호에 기호당 1비트 이상(예: 2비트)이 있는 경우, SigDigger는 **어떤 기호가** 00, 01, 10, 11인지 알 수 없으므로 서로 다른 **회색 음영**을 사용하여 각각을 나타냅니다(비트를 복사하면 **0에서 3까지의 숫자**를 사용하므로 이를 처리해야 합니다).
