...
后端多次信任 **Host header** 来执行某些操作。例如，它可能会使用其值作为 **发送密码重置的域名**。因此，当您收到一封包含重置密码链接的电子邮件时，使用的域名就是您在 Host header 中输入的域名。然后，您可以请求其他用户的密码重置，并将域名更改为您控制的域名，以窃取他们的密码重置代码。[WriteUp](https://medium.com/nassec-cybersecurity-writeups/how-i-was-able-to-take-over-any-users-account-with-host-header-injection-546fff6d0f2)。
