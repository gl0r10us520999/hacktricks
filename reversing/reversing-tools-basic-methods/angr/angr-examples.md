...
main(sys.argv)
```
В цьому сценарії вхідні дані були отримані за допомогою `scanf("%u %u")`, і було введено значення `"1 1"`, тому значення **`0x00000001`** у стеку походять з **введення користувача**. Ви можете побачити, як ці значення починаються з `$ebp - 8`. Отже, в коді ми **відняли 8 байт від `$esp` (оскільки в той момент `$ebp` і `$esp` мали однакове значення)**, а потім ми помістили BVS.
