...
Ako završite u kodu **koristeći pomeranje udesno i ulevo, xore i nekoliko aritmetičkih operacija** vrlo je verovatno da je to implementacija **kriptografskog algoritma**. Ovde će biti prikazani neki načini da se **identifikuje algoritam koji se koristi bez potrebe da se obrne svaki korak**.
...
Iz [dokumentacije](https://learn.microsoft.com/en-us/windows/win32/api/wincrypt/nf-wincrypt-cryptacquirecontexta): Funkcija **CryptAcquireContext** se koristi za sticanje rukohvata za određeni kontejner ključeva unutar određenog pružatelja kriptografskih usluga (CSP). **Ovaj vraćeni rukohvat se koristi u pozivima funkcija CryptoAPI** koje koriste odabrani CSP.
...
* Retko je pronaći neki malware koji ga koristi, ali postoje primeri (Ursnif)
...
Stoga, moguće je identifikovati ovaj algoritam proveravajući **magični broj** i **početne XOR-ove**, videći **veoma dugu funkciju** i **upoređujući** neke **instrukcije** duge funkcije **sa implementacijom** (kao što su pomeranje ulevo za 7 i rotacija ulevo za 22).
...
Možete identifikovati oba proveravajući konstante. Napomena da sha\_init ima 1 konstantu koju MD5 nema:
