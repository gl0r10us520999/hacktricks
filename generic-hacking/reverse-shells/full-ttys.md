...
Zauważ, że powłoka, którą ustawisz w zmiennej `SHELL` **musi** być **wymieniona w** _**/etc/shells**_ lub `Wartość zmiennej SHELL nie została znaleziona w pliku /etc/shells. Ten incydent został zgłoszony`. Również zauważ, że następne fragmenty działają tylko w bash. Jeśli jesteś w zsh, przełącz się na bash przed uzyskaniem powłoki, uruchamiając `bash`.
